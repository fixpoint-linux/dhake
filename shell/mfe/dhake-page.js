// shell/mfe/dhake-page.js — the Elm dhake docs page as an @mfe MFE module.
//
// Implements the @mfe/core MFE lifecycle ({ mount, unmount, update }) so the
// shell's reconcile can mount the Elm app into a [data-mfe="dhake-page"]
...
function loadElm() {
  if (!elmPromise) {
    elmPromise = (async () => {
      ...
      if (!res.ok) {
        throw new Error(`dhake-page: failed to fetch ${ELM_URL} (HTTP ${res.status})`);
      }
      const code = await res.text();
      const Elm = evalBundle(code);
      if (!Elm || !Elm.Main || typeof Elm.Main.init !== 'function') {
        throw new Error('dhake-page: dist/elm.js did not expose Elm.Main.init');
      }
      return Elm;
    })().catch((err) => {
